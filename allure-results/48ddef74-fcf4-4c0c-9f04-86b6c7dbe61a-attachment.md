...
Call log:
  ...
  - waiting for getByText('#addmessage')
...
```ts
  ...
  7  |     console.log(await expect(page.getByPlaceholder("Please enter first value")).toBeVisible());
  8  |     await page.getByPlaceholder("Please enter first value").pressSequentially("15");
...
> 12 |     console.log(await expect(page.getByText("#addmessage")).toContainText("35"));
     |                                                             ^ Error: expect(locator).toContainText(expected) failed
  13 |     page.close();
  14 | });
  15 | 
