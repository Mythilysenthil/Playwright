...
```ts
  ...
  8  |     await page.getByPlaceholder("Please enter your Message").isVisible();
  9  |     await page.getByPlaceholder("Please enter your Message").fill("Mythily");
  10 |     await page.getByRole('button', {name: 'Get Checked Value'}).click();
> 11 |     await expect(page.getByText("Mythily")).toContainText("Mythily")
     |                                             ^ Error: expect(locator).toContainText(expected) failed
  12 | });
```