...
- Name: Simpledemo.test.ts >> Login Test
...
```ts
  ...
  6  |     await page.getByPlaceholder("Please enter your Message").isVisible();
> 7  |     await page.locator("#user-message").fill("Mythily");
     |                                         ^ Error: locator.fill: Error: strict mode violation: locator('#user-message') resolved to 3 elements:
  8  |     await page.getByRole('button', {name: 'Get Checked Value'}).click();
  9  |     await expect(page.getByText("Mythily")).toContainText("Mythily")
  10 | });
```